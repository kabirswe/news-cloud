...
import {editMod, approveMod, prohibitMod, commentFetchInterval} from '../../../../app-constants';
...
import {  SaveIcon, DeleteIcon} from '../../mod-assets/svgComp';
...
*/
  ...
  onEditTabSeoChange = (value) => {
    // let value = event.target.value
    let {article, article:{editTab}, articleError, maxLen, initialMaxLen} = this.state;
    let totalLen = value && value.join('').length;
     ...
     if (value.length < editTab.seoTags.length) {
      ...
      this.setState({
        maxLen: {
          ...maxLen,
          seoTags: ((initialMaxLen.seoTags - totalLen) >= this.tagsMaxValue) ? 
          this.tagsMaxValue : removed.length
        }
      })
      
    } else {
      let characterLeft  = (initialMaxLen.seoTags - totalLen) >= this.tagsMaxValue ? 
      this.tagsMaxValue : initialMaxLen.seoTags - totalLen
      this.setState({
        maxLen: {
          ...maxLen,
...
*/
  ...
  onChangePublishApproval = (event) => {
    ...
    if (
      ...
      this.setState({
        showWarningMessage: contentModtranslator('ARTICLE_EDIT.ARTICLE_NOT_PUBLISHED_YET'),
        warningModal: true
      });
      return;
    }
  ...
  onChangeProhibitApproval = (event) => {
    ...
    this.setState({
      ...
      fields: {
        ...
        preview: Number(checked) ? (isApprover || isPublisher) ? false : true  : false,
    ...
    AxiosService.post(ApiService.SAVE_ARTICLE, prepareArticelData, false, false)
      .then((result) => {
        if (result.data) {
          ...
          // window.history.replaceState(null, null, `/copy-content?aid=${this.id}`)
      ...
      AxiosService.put(
        ApiService.UPDATE_ARTICLE(articleId),
        prepareArticelData,
        false,
        false
      )
...
saveArticleData = (reScheduleMail, snsList) => {
  ...
  let prepareArticelData = {
    ...
    isProhibitedToPublish: article.manageMentTab.prohibition,
    isStoppedToPublish: article.manageMentTab.approvalCancelation,
    sourceArticleId: Number(this.copyId)
  };
  ...
  confirmReviewRequest = () => {	
    let {article:{manageMentTab}, workInProgress:{approverRequestInprogress}, baseArticle, articleData} = this.state	
  ...
  toggleMailScheduleModal = (value) => {
    let {article:{startDate}, mailPosting} = this.state;
    if (value == 'save') {
      if (mailPosting.id) {
  ...
  toggleMailApprovalModal = (value) => {
    let {article:{startDate}} = this.state;
    if (value == 'save') {
  ...
  confirmApproveRequest = () => {	
    let {article:{manageMentTab}, workInProgress:{approverRequestInprogress}, baseArticle, articleData} = this.state	
    AxiosService.get(ApiService.REQUEST_APPROVAL(manageMentTab.approvalRequest, this.id))	
    .then(result => {	
      Toast.success(result.data.message)	
      this.fetchAllComment()	
      this.setState({	
  ...
  componentDidMount() {
    ...
    let {article} = this.state;
    let {id, aid} = queryString.parse(window.location.search);
    this.copyId= id
    ...
    AxiosService.get(ApiService.GET_ARTICLE(aid || id), false)
      .then((result) => {
        if (result.data.data) {
          let {article, maxLen, initialMaxLen} = this.state
          let data = result.data.data
          let selectedCat = data.article_category;


          let editorState =  data.article_body && JSON.parse(data.article_body) && EditorState.createWithContent(
            convertFromRaw(JSON.parse(data.article_body)) || {}
          )
          
          let art = {}
          // after copy if user save the article then
          if (aid) {
            let afterSave = {
              title: data.title,
              url: data.url,
              ...
              startDate: this.state.article.startDate,
              endDate: '',
              editTab: {
                category: [],
          ...
          this.setState({
           ...
           selectedCategory: [{value: selectedCat.id, label: selectedCat.name}]
          });
        }
      })
      .catch((error) => {
  ...
  commentTemplate (data={}) {
    ...
    if (data.comment_type == 2) {
      ...
      if (data.is_approve && data.is_review) {
        text = <div>
        <span>{data.from_user && data.from_user.username}さん</span>{' '}
      がレビュー<span>{status[st]}</span>, 承認 <span>{status[st]}</span> しました。
      </div>
      }else if (data.is_review) {
      text =  <div>
      <span>{data.from_user && data.from_user.username}さん</span>{' '}
      がレビュー<span>{status[st]}</span>しました。
    </div>
      } else {
        text = <div>
        <span>{data.from_user && data.from_user.username}さん</span>{' '}
        承認 <span>{status[data.approval_status]}</span> しました。
      </div>
      }
      return (
        <>
    ...
    if (data.comment_type == 3) {
      ...
      if (data.is_approve) {
        text = <div>
        <span>{data.from_user && data.from_user.username}さんが {user.user && user.user.username}さん</span> {approveRequestText}
      </div>
      }
      if (data.is_review) {
        text = <div>
        <span>{data.from_user && data.from_user.username}さんが {user.user && user.user.username}さん</span> {reviewRequestText}
      </div>
      }
      return (
        <>
...
findCategory = (selectedCategory = [], categoryList) => {
  ...
  if (selectedCategory && selectedCategory.length) {
    ...
    let cat = categoryList.find(cat => cat.id == selected.id)
      ...
      {isLoader ? (
        ...
        />  */}
          ...
          <ConfirmationModal 
              ...
              body={
                <>
                  <div>{articleWarning.startDate ? articleWarning.startDate : ''}</div>
          ...
          <ConfirmationModal 
              ...
              title={contentModtranslator('ARTICLE_EDIT.STOP_TO_PUBLISH_CONFIRMATION_TITLE')}
              body={contentModtranslator('ARTICLE_EDIT.STOP_TO_PUBLISH_CONFIRMATION')}
            ...
            <div className="custom-row">
              <div className="left-side">
                      ...
                      <div className="date-range">
                        <div className="left-date">
                          <DateInput
                            ...
                            disabled={
                              ...
                              !this.approvedOnlyPermission(approveMod.type.date)) || fields.startDate
                            }
                            maxDate={article.endDate ? article.endDate : null}
                            type="startDate"
                            value={article.startDate}
                            selected={this.dateParseSafari(article.startDate)}
                            showTimeSelect
                            timeFormat="HH:mm"
                            timeIntervals={15}
                            timeCaption="time"
                        ...
                        <div className="right-date">
                          <DateInput
                            ...
                            disabled={
                              (this.isArticleEditable() &&
                              !this.approvedOnlyPermission(approveMod.type.date)) || fields.endDate
                            }
                            showTimeSelect
                            handleDate={this.handleDateChange}
                      ...
                      <div className="management-form">
                        ...
                        <div className="request-input">
                          ...
                          <Select
                            ...
                            isLoading={reviewerList ? reviewerList.length ? false : true : false}
                        ...
                        <div className="request-input">
                          ...
                          <Select
                           ...
                           options={approverList || []}
                            ...
                            isLoading={approverList ? approverList.length ? false : true : false}
                            className="approver-select"
                            classNamePrefix="approver"
                            components={{DropdownIndicator}}
                            name=""
                        ...
                        <div className="request-input">
                          ...
                          <NcThreeStateButton
                            ...
                            disabled={this.isReviewEnabled() || fields.review || !articleId}
                        ...
                        <div className="request-input">
                          ...
                          <NcThreeStateButton
                             ...
                             disabled={this.isApproveEnabled() || fields.approve || !articleId}
                        ...
                        <div className="request-input">
                            <NcCheckbox
                             disabled={this.isStopPublishEnabled() && fields.stopPublish || !articleId}
                            name="approvalCancelation"
                            id="setting-checkobox4"
                            handleChange={this.onChangePublishApproval}
                            checked={manageMentTab.approvalCancelation? true : false}
                          />
                            <div className="btn-label">
                            {contentModtranslator(
                              'ARTICLE_EDIT.SUSPENSION'
                            )}{' '}
                          </div>
                        </div>
                        <div className="request-input">
                          <NcCheckbox
                            disabled={this.isProhibitEnabled() || fields.prohibit || !articleId}
                            name="prohibition"
                ...
                <div className="edit-action-btn">
                  ...
                  <button
                    ...
                    className={
                      (!this.prohibitOnlyPermission(prohibitMod.type.save) && fields.save)
                  ...
                  <button
                    ...
                    className={!articleId || fields.delete ?"delete-btn warning-color disabledBtn" : "delete-btn warning-color"}
                  >
                    <DeleteIcon/>
              ...
              <div className="right-side">
                ...
                <StatusIconWithLabel
                    ...
                    labelText={ approvalIcon[articleData.custom_approving_status]
                ...
                <StatusIconWithLabel
                    ...
                    color={
                      ...
                      ? publicationIcon[articleData.custom_publishing_status].COLOR
                      : publicationIcon[1].COLOR
                    }
                    labelText={ publicationIcon[articleData.custom_publishing_status]
                      ? publicationIcon[articleData.custom_publishing_status]
                          .STATUS
